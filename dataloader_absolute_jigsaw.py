...
def generate_permutations(perm_number=30, grid_size=3*3, seed=300):
  random.seed(seed)
...
permutations = generate_permutations(perm_number=30)
...
class DatasetGeneratorMultimodal(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            if self.permute:
              modality = random.choice(["rgb", "depth"])
              if modality == "rgb":
                perm_index_rgb = random.choice(list(range(30)))
              else:
                perm_index_depth = random.choice(list(range(30)))
        ...
        if self.permute and (self.transform is None):
          if perm_index_rgb is None:
            return img_rgb, img_depth, lab, perm_index_depth
